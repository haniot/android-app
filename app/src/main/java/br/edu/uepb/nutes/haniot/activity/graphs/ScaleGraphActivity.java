...
public class ScaleGraphActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    List<MeasurementScale> measurementData;
    ...
    private void createChart(int type) {
        ...
        if(type == GRAPH_TYPE_DAY) {
            ...
            measurementData= MeasurementScaleDAO.getInstance(this).filter(DateUtils.addDays(0).getTimeInMillis(), getDateTime(0), deviceAddress, session.getIdLogged());
